...
import PicturePreviewCard from "./admin/PicturePreviewCard";
import { addTagsPost, getAllImagesTags } from "../redux/admin.images.slice";
...
const Dashboard = () => {
  ...
  const {data,error,isLoading}=useSelector(ss=>(ss?.adminImages?.imagesData))
  if(error)
  {
    message.error(error);
  }
  ...
  useEffect(()=>{
   dispatch(getAllImagesTags({page,limit, selectedCategory, labels:selectedLabels}))
      ...
      <div className="">
        ...
        <Title level={5}>Explore Categories</Title>
            ...
            <div className="mb-4 flex justify-between ">
              ...
              <Link to="add" >
              <Button className="bg-blue-600 text-white" >Add Image</Button>
              </Link>
            ...
            <div className="flex gap-4 justify-between flex-wrap h-fit">
              ...
              (
                  ...
                  <div className="w-[30%]">
                     <PictureCard key={image?.id} id={image?.id} image={image?.data?.images} labels={image?.data?.labels} selectedCategory={image?.data?.category} page={page} limit={limit}   />
                  </div>
